...
# Tạo agent Monte Carlo và đọc Q-table từ file đã lưu
agent = MonteCarloAgent(alpha=0.0, gamma=0.0, epsilon=0.0)  # Không cần học thêm, chỉ khai thác

# Đọc Q-table đã huấn luyện từ file
with open('q_table_montecarlo_100000.json', 'r') as json_file:
    q_table_serializable = json.load(json_file)

# Chuyển Q-table từ string keys sang tuple keys
